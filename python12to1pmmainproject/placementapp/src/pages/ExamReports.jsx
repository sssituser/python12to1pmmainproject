            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{selectedResult.totalQuestions}</p>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{selectedResult.totalQuestions * 2}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-gray-600">Your Score</p>
                  <p className="text-2xl font-bold text-gray-900">{selectedResult.correctAnswers * 2}/{selectedResult.totalQuestions * 2}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex items-center justify-between">
                  ...
                  <i className="bi bi-clock text-blue-500 text-2xl"></i>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-6">
                ...
                <div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${selectedResult.totalQuestions > 0 ? (selectedResult.correctAnswers / selectedResult.totalQuestions) * 100 : 0}%` }}
                    ></div>
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-gray-900">
                      {Math.round((selectedResult.answers.filter(a => a !== null).length / selectedResult.totalQuestions) * 100)}%
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-green-600 h-2 rounded-full" 
                      style={{ width: `${(selectedResult.answers.filter(a => a !== null).length / selectedResult.totalQuestions) * 100}%` }}
                    ></div>
                  </div>
                </div>
              </div>
              ...
              {selectedResult.reason && (
                <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-sm font-semibold text-red-800 mb-1">Reason for Failure:</p>
                  <p className="text-sm text-red-700">{selectedResult.reason}</p>
                </div>
              )}
            </div>
                            ...
                            <div className="mt-1 space-y-1">
                              {question.options.map((option, optIndex) => (
                                <div key={optIndex} className={`p-2 rounded ${
                                  optIndex === question.correct ? 'bg-green-100 border border-green-300' :
                                  optIndex === userAnswer && !isCorrect ? 'bg-red-100 border border-red-300' : 'bg-gray-50'
                                }`}>
                                  <span className="font-medium">{String.fromCharCode(65 + optIndex)}.</span> {option}
                                  {optIndex === question.correct && <span className="ml-2 text-green-600 font-semibold">✓ Correct</span>}
                                  {optIndex === userAnswer && !isCorrect && <span className="ml-2 text-red-600 font-semibold">✗ Your Answer</span>}
                                </div>
                              ))}
                            </div>
                          </div>
                  ...
                  <div className={`p-6 border-t-4 ${
                    ...
                    <div className="mb-4">
                      ...
                      <div className={`text-sm font-semibold ${
                        ...
                        {percentage.toFixed(1)}% - {result.status === 'fail' ? 'Failed' : passed ? 'Passed' : 'Completed'}
                      </div>
                    </div>
