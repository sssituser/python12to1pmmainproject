...
function Jobs() {
  ...
  function fetchJobs() {
    ...
    fetch(`http://${window.location.hostname}:8000/api/admin/jobs/`, {
...
function createJob(e) {
  ...
  const url = form.id
    ? `http://${window.location.hostname}:8000/api/admin/jobs/${form.id}/`   // UPDATE
    : `http://${window.location.hostname}:8000/api/admin/jobs/`;             // CREATE
...
fetch(url, {
  ...
  function deleteJob(id) {
    ...
    fetch(`http://${window.location.hostname}:8000/api/admin/jobs/${id}/`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` }
    })
      .then((res) => {
