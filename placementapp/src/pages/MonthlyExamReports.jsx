...
import { ArrowLeft, BarChart3, Clock, BarChart, User } from "lucide-react";
...
function MonthlyExamReports() {
  ...
  const fetchReports = async (showLoading = false) => {
    try {
      ...
      const url = `http://${window.location.hostname}:8000/api/all-exam-results/?exam_type=monthly&username=${currentUsername}`;
      ...
      } catch (e) { console.error("Monthly backend fetch failed", e); }
      
      const localList = JSON.parse(localStorage.getItem("allExamResults") || "[]");
      const examList = [...localList.filter(e => (e.examType || e.exam_type || '').toLowerCase() === 'monthly'), ...backendList];
      ...
      examList.forEach(exam => {
         ...
         if (type !== 'monthly' && !title.includes('monthly')) return;
         ...
         const displayTitle = (exam.examTitle || exam.exam_title || "Monthly Exam").toLowerCase();
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      exams.forEach((exam, index) => {
        ...
        const total = (exam.total_marks ?? exam.totalMarks ?? (exam.totalQuestions || 30)) || 30;
    ...
    <div className="min-h-screen bg-[#F8FAFC] py-8 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <button
          ...
          className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-purple-600 transition-all mb-8 group bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.2em] text-purple-600 mb-1">SSSIT Assessment Analysis</p>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-2">
              Monthly <span className="text-purple-600">Mastery</span>
            </h1>
            <p className="text-slate-500 font-medium">Comprehensive review of your monthly assessments.</p>
          </div>
        ...
        {loading && exams.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            <div className="w-16 h-16 border-4 border-purple-600 border-t-transparent rounded-full animate-spin shadow-lg"></div>
            <p className="text-slate-400 font-black text-xs uppercase tracking-[0.3em] animate-pulse">Analyzing Data...</p>
          </div>
        ) : exams.length > 0 ? (
                ...
                >
                  ...
                  <div className="w-full mb-8 text-center">
                    <h3 className="text-xl font-black text-slate-900 truncate mb-1 uppercase tracking-tight">
                       {exam.examTitle || exam.exam_title || `Monthly Assessment #${exams.length - index}`}
                    </h3>
                  </div>
                  ...
                  <div className="w-full grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-slate-50 p-3 rounded-2xl text-center border border-slate-100/50">
                       <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Final</p>
                  ...
                  <div className="w-full flex items-center gap-3 text-slate-400 mb-8 border-t border-slate-50 pt-6 text-center justify-center">
                     ...
                     <span className="text-xs font-bold whitespace-nowrap">
                        {exam.examDate ? new Date(exam.examDate).toLocaleDateString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "Local record"}
                     </span>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-purple-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-purple-800 transition-all active:scale-95 shadow-lg shadow-purple-100 flex items-center justify-center gap-2 group/btn"
                  >
          ...
          <div className="bg-white rounded-[3rem] p-16 text-center shadow-xl shadow-slate-200/50 border border-slate-100 mt-8 flex flex-col items-center">
            <div className="w-24 h-24 bg-purple-50 rounded-full flex items-center justify-center text-purple-600 mb-8">
               <BarChart size={48} />
            </div>
            <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tight uppercase">No Monthly Reports</h3>
            <p className="text-slate-400 font-medium max-w-md mx-auto leading-relaxed mb-10">
               Your monthly analytical report will be generated after your next major assessment. Keep practicing!
            </p>
            <button 
              onClick={() => navigate("/dashboard/playground")}
              className="px-10 py-4 bg-purple-600 text-white rounded-full font-black uppercase tracking-widest hover:bg-purple-700 transition-all shadow-xl shadow-purple-100"
            >
              Start Assessing
            </button>
          </div>
